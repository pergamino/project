...
# on re-simule a partir de 999 (S$Meteo$Date[1000] => "1981-09-26 UTC"):
# 1) a l'identique :
...
plot(S0$Sim$LAI~ S0$Meteo$Date,type="l",col="green",lwd=1,xlab="time",ylab="LAI")
...
lines(S0$Sim$CM_RE~ S0$Meteo$Date,type="l",col="red",lwd=1,xlab="time",ylab="reserve2")
...
ggplot()+
  ...
  geom_point(data=S0$Sim,aes(x=S0$Meteo$Date,y=CM_Fruit),color="blue")+
  geom_point(data=S0$Sim,aes(x=S0$Meteo$Date,y=Harvest_Fruit),color="red")
