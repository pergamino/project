...
coffee= function(){
  list(
    ...
    MeanAgePruning    = 2,          # Age of first pruning (year)
    LeafPruningRate   = 0,        #0.6 how much leaves are pruned (ratio)
    WoodPruningRate   = 0,        #1/3 how much branches wood are pruned (ratio)
    ...
    harvest           = "quantity", # Harvest condition: 
            #"quality"  -> harvest when most fruits are mature is reached (optimize fruit quality)
   ...
   # Min_Fruit_CM      = 10,         # Minimum fruit carbon mass below which harvest cannot be triggered
    ...
    epsilon_Shoot     = 1.20,       # Resprout wood growth respiration coefficient (g g-1). Source: Dufr?ne et al. (2005)
