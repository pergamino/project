...
incidencia <- select(incidencia,-c(Infeccion))
...
nn.rp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_observada + incidencia_pronosticada + error_t,
...
nn.bp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_observada + incidencia_pronosticada + error_t,
                   data = incidencia.norm[train,],
                   hidden = 5,
                   threshold = 0.01,
                   stepmax = 1e7,
