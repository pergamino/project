...
incidencia <- select(incidencia,-c(Infeccion,incidencia_observada))
...
nn.rp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_pronosticada + error_t,
...
nn.bp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_pronosticada + error_t,
                   data = incidencia.norm[train,],
                   hidden = 6,
                   threshold = 0.01,
                   stepmax = 1e7,
