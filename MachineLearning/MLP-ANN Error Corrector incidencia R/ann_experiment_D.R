...
incidencia <- select(incidencia,-c(Infeccion,error_t))
...
nn.rp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_observada + incidencia_pronosticada,
...
nn.bp <- neuralnet(formula = error_t1 ~ Lavado + incidencia_observada + incidencia_pronosticada,
                   data = incidencia.norm[train,],
                   hidden = 5,
                   threshold = 0.01,
                   stepmax = 1e7,
