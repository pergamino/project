...
func_ipsim <- function(output_moulinette,
  ...
  for(j in 1:nrow(sub_df)){
    ...
    sub_output_incidencia <- subset(output_incidencia,Fecha_median==sub_df$fecha_med_monitoreo[j])
    
    for(k in 1:(nrow(sub_output_incidencia)/2)){
      
    # Inoculum de inoculo (para el parasitismo y eficacia fungicidas) : 
    # 1: bajo (inc<10%), 2: medio (inc [10-30%]), 3 : alto (inc>30%) 
    cant_inoc = subset(sub_output_incidencia,var=="Inoculo")$cat[k]
    ...
    # 2: Efecto medio 
    # 3: Efecto alto 
    ef_inc_defol = subset(output_incidencia,var=="Efecto de la incidencia sobre la defoliacion")$cat[k]
    ...
    for (efah in 1:nrow(df19)){
      ef_fruto_creci[k] <- if (df19$Fenologia_del_fruto[efah]==feno_cafe[j] & 
    ...
    for (iefc in 1:nrow(df18)){
      int_fruto_clim[k] <- if (df18$Efecto_del_fruto_sobre_el_crecimiento_vegetativo[iefc]==ef_fruto_creci[k] &
    ...
    for (ahnc in 1:nrow(df17)){
      ap_hoj_nutri_clim[k] <- if (df17$Interaccion_fruto_clima_sobre_el_crecimiento_vegetativo[ahnc]==int_fruto_clim[k] &
    ...
    for (ah in 1:nrow(df16)){
      aparicion_hojas[k] <- if (df16$Interaccion_fruto_clima_nutricion_sobre_el_crecimiento_vegetativo[ah]==ap_hoj_nutri_clim[k] &
    ...
    for (efd in 1:nrow(df15)){
      ef_fruto_def[k] <- if (df15$Fenologia_del_fruto[efd]==feno_cafe[j] & 
    ...
    for (d in 1:nrow(df14)){
      defoliacion[k] <- if (df14$Incidencia_sobre_la_defoliacion[d]==ef_inc_defol &
                            df14$Efecto_del_fruto_sobre_la_defoliacion[d]==ef_fruto_def[k]){
        df14$Defoliacion[d]
      } else {
        next
      }
    ...
    for (sc in 1:nrow(df13)){
      suscept_lig_carga[k] <-  if (df13$Fenologia_del_fruto[sc]==feno_cafe[j] & 
    ...
    for (shs in 1:nrow(df12)){
      suscept_hosp_sombr[k] <-  if (df12$Efecto_del_fruto_sobre_la_susceptibilidad_de_la_hoja_a_la_roya[shs]==suscept_lig_carga[k] & 
    ...
    for (sh in 1:nrow(df11)){
      suscept_hosp[k] <-  if (df11$Susceptibilidad_fisiologica_del_hospedero[sh]==suscept_hosp_sombr[k] & 
    ...
    for (late in 1:nrow(df10)){
      latenc[k] <- if(df10$Latencia_por_temperatura[late]==lat &
    ...
    for (ifcc in 1:nrow(df9)){
      infecc[k] <- if(df9$Infeccion_de_las_hojas_por_mojadura[ifcc]==inf &
    ...
    for (iddp in 1:nrow(df8.1)){
      inoc_disp_desp_parasit[k] <- if(df8.1$Sombra[iddp]==sombra &
    ...
    for (pe in 1:nrow(df8.2)){
      perd_esp[k] <- if(df8.2$Perdidas_de_esporas_por_lavado[pe]==perd &
    ...
    for (cspor in 1:nrow(df7)){
      cant_esp_disp[k] <- if(df7$Inoculo_disponible_despues_del_parasitismo[cspor]==inoc_disp_desp_parasit[k] & 
                             df7$Perdidas_de_esporas[cspor]==perd_esp[k]) {
    ...
    for (cr in 1:nrow(df6)){
      creci[k] <-  if (df6$Cantidad_de_esporas_disponible_para_la_infeccion[cr]==cant_esp_disp[k] &
                       df6$Infeccion[cr]==infecc[k] &
                       df6$Latencia[cr]==latenc[k]) {
    ...
    for (eaf in 1:nrow(df5)){
      ef_ap_fungicidas[k] <-  if (df5$Cantidad_de_inoculo[eaf]==cant_inoc &
    ...
    for (ichp in 1:nrow(df4)){
      int_clim_hosp_patog[k] <-  if (df4$Crecimiento_potencial_de_la_poblacion_patogenica[ichp]==creci[k] &
                                     df4$Susceptibilidad_del_hospedero[ichp]==suscept_hosp[k]
      ) {
    ...
    for(epp in 1:nrow(df3)){
      evo_pobla_patogenica[k] <- if (df3$Interaccion_clima_hospedero_patogeno[epp]==int_clim_hosp_patog[k] & 
                                     df3$Efficacia_de_la_aplicacion_de_las_fungicidas[epp]==ef_ap_fungicidas[k]){
    ...
    for (rsgap in 1:nrow(df2)){
      riesgo_antes_poda[k] <-  if (df2$Evolucion_de_la_poblacion_patogenica[rsgap]==evo_pobla_patogenica[k] &
                                   df2$Aparicion_de_las_hojas[rsgap]==aparicion_hojas[k] & 
                                   df2$Defoliacion[rsgap]==defoliacion[k]) {
    ...
    for (rsg in 1:nrow(df1)){
      riesgo[k] <-  if (df1$Poda[rsg]==poda_del_mes &
                        df1$Riesgo_de_crecimiento_de_la_incidencia_antes_de_la_poda[rsg]==riesgo_antes_poda[k]) {
    ...
    sortie_ipsim_t <- data.frame(
      Fecha_median=sub_output_incidencia$Fecha_median[k],
      incidencia=sub_output_incidencia$incidencia[k],
      riesgo=riesgo[k],
      categoria=categorie[riesgo[k]],
      inicial_categoria = initiale_categorie[riesgo[k]],
      Perdidas_de_esporas=perd_esp[k],
      Infeccion = infecc[k],
      Latencia = latenc[k],
      Aparicion_hojas = aparicion_hojas[k],
      Defoliacion=defoliacion[k],
      Cantidad_esporas=cant_esp_disp[k],
      Suscept_hospedero=suscept_hosp[k],
      Sombra=sombra,
      id_inc=sub_output_incidencia$id_inc[k]
      )
   ...
   sortie_ipsim_ori <- list(sortie_ipsim,tabla_arbol)
    }
