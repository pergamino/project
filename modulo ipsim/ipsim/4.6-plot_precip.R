...
# Plot
 ...
 p <-
   ...
   annotation_custom(
     grob = linesGrob(gp=gpar(col="darkgreen", lwd=3)), 
     xmin = as.Date(max(as.mondate(sub_df_meteo2$fecha0)) + 2.6), 
     xmax = as.Date(max(as.mondate(sub_df_meteo2$fecha0)) + 2.6),  
     ymin = 0*max(sub_df_meteo2$precip)/max(sub_dia_lav$num_dia), 
     ymax = 2.9*max(sub_df_meteo2$precip)/max(sub_dia_lav$num_dia)
   )+
   ...
   annotation_custom(
     ...
     xmin = as.Date(max(as.mondate(sub_df_meteo2$fecha0)) + 3.3), 
     xmax = as.Date(max(as.mondate(sub_df_meteo2$fecha0)) + 3.3), 
     ymin = 0*max(sub_df_meteo2$precip)/max(sub_dia_lav$num_dia),
     ymax = 2.9*max(sub_df_meteo2$precip)/max(sub_dia_lav$num_dia) 
