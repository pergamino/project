...
TME<-function(myData0, 
  ...
  vgam.salida_ajust_all <- NULL
  
  YEAR <- unique(myData0$Year)
  
 for(yr in 1:length(YEAR)){
   
   Sys.sleep(1)
   if(progress)
     incProgress(1/length(YEAR))

   data <- subset(myData0,Year==YEAR[yr])
   
  ###Tabla con umbrales de riesgo teniendo en comun los meses (Num_mes) y categorias
  umbral <- subset(data,select=c(FACTOR,Num_mes,num_detect))

  # CAT<-unique(data[,lfactor])
  CAT<-unique(data$FACTOR)
  ...
  for(j in 1:length(CAT)) {
    ...
    dts<-data.frame(Num_mes=subset(data,FACTOR==CAT[j],select=Num_mes),
                    HR=subset(data,FACTOR==CAT[j],select=Hojas.con.Roya),
                    HE=subset(data,FACTOR==CAT[j],select=Hojas.Evaluadas))
    names(dts) <- c("Num_mes","HR","HE")
    
    FEC<-unique(dts$Num_mes)
    
    sal<-data.frame(matrix(nrow=length(FEC), ncol=8))
    colnames(sal)<-c('N.obs', 'Num_mes','TM', 'Metodo.est','p','ee','rho','Q.agregacion')
  ...
  vgam.salida_ajust <- merge(vgam.salida,df_Np,by="FACTOR",all.x = T)
  
  vgam.salida_ajust$TM_ajustado <- vgam.salida_ajust$TM*vgam.salida_ajust$Np/(vgam.salida_ajust$Np-1+vgam.salida_ajust$TM)

  vgam.salida_ajust$Year <- unique(data$Year)
  
  vgam.salida_ajust_all <- rbind(vgam.salida_ajust_all,vgam.salida_ajust)

  
  }


  return(vgam.salida_ajust_all)
}