...
if resultrefresh.status_code == 200:
    ...
    resultdata = requests.post(pwregurl + "/api/replicacion/getDatosReplicar", json={"codPais":2}, verify=True, headers=h)
    data = resultdata.json()
    resultrefresh = requests.post(pwpaiurl + "/api/authentication/refresh", verify=True, json={"OldToken":pwpaitoken,"RefreshToken":pwpairefreshtoken}, headers={"Content-Type":"application/json"})
    if resultrefresh.status_code == 200:
        ntoken = resultrefresh.text
        h = {
            "Authorization": "Bearer " + ntoken,
        ...
        resultdata = requests.post(pwpaiurl + "/api/replicacion/saveDatosAppReplicar", json=data, verify=True, headers=h)
        data = resultdata.json()
        if data == 0:
            resultrefresh = requests.post(pwregurl + "/api/authentication/refresh", verify=True, json={"OldToken":pwregtoken,"RefreshToken":pwregrefreshtoken}, headers={"Content-Type":"application/json"})
            if resultrefresh.status_code == 200:
                ...
                resultdata = requests.post(pwregurl + "/api/replicacion/clearReplicados", json=data, verify=True, headers=h)